    ...
    for entry in report_data['trackingData']:
        if 'mediaTime' in entry:
            ...
            if entry['type'] == 'FACE_ATTENTION':
                ...
                valence_values.append(normalize_value(entry['valence']))  # Rescale valence
...
def plot_timeline(media_times, attention_values, valence_values, arousal_values, file_name, output_dir):
    plt.figure(figsize=(19.2, 10.8))  # Set the horizontal size to 1920px (19.2 inches with 100 dpi)
    ...
    plt.plot(media_times[:len(valence_values)], valence_values, label="Valence", color='orange')
    plt.plot(media_times[:len(arousal_values)], arousal_values, label="Arousal", color='red')
...
def export_to_csv_xlsx(media_times, attention_values, valence_values, arousal_values, file_name, output_dir):
    # Ensure that arrays are of the same length
    media_times, attention_values, valence_values, arousal_values = pad_arrays_to_length(media_times, attention_values, valence_values, arousal_values)
...
def plot_summary(media_times_combined, attention_combined, valence_combined, arousal_combined, output_dir):
    ...
    plt.plot(media_times_combined, attention_combined, label="Attention", color='cyan')
    plt.plot(media_times_combined, valence_combined, label="Valence", color='orange')
...
def process_all_json_files(directory):
    ...
    for file_path in json_files:
        ...
        media_times, attention_values, valence_values, arousal_values = process_report(file_path)
        plot_timeline(media_times, attention_values, valence_values, arousal_values, file_name, output_dir)
        export_to_csv_xlsx(media_times, attention_values, valence_values, arousal_values, file_name, output_dir)
        
        # Append data to the combined lists for the summary
        media_times_combined.extend(media_times)
        attention_combined.extend(attention_values)
        valence_combined.extend(valence_values)
        arousal_combined.extend(arousal_values)
    
    # Ensure all arrays are of the same length for the summary report
    media_times_combined, attention_combined, valence_combined, arousal_combined = pad_arrays_to_length(media_times_combined, attention_combined, valence_combined, arousal_combined)
    
    # Create the summarized report with grid cells
    plot_summary(media_times_combined, attention_combined, valence_combined, arousal_combined, output_dir)
    
    # Export combined data to CSV and Excel for summary
    export_to_csv_xlsx(media_times_combined, attention_combined, valence_combined, arousal_combined, "summarized", output_dir)
